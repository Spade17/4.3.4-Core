...
class ByteBuffer
{
    public:
        ...
        bool WriteBit(uint32 bit)
        {
            ...
            if (_bitpos == 0)
            {
				sLog->outInfo(LOG_FILTER_NETWORKIO, "WriteBit bit %u", bit);
                _bitpos = 8;
        ...
        void WriteGuidMask(uint8 order)
        {
			sLog->outInfo(LOG_FILTER_NETWORKIO, "WriteGuidMask order : %u result %u", order, _guidBytes[order]);
			WriteBit(uint32(_guidBytes[order]));
        }

		void WriteGuildGuidMask(uint8 order)
		{
			sLog->outInfo(LOG_FILTER_NETWORKIO, "WriteGuildGuidMask order : %u result %u", order, _guildGuidBytes[order]);
			WriteBit(uint32(_guildGuidBytes[order]));
		}

		void WriteGuidByte(ByteBuffer* dataBuffer, uint8 order)
		{
			dataBuffer->WriteBit(0);
			return;
		}

		void WriteGuildGuidByte(ByteBuffer* dataBuffer, uint8 order)
		{
			dataBuffer->WriteBit(0);
			return;
		}
        ...
        */
		...
		{
			memcpy(_guidBytes, &guid, sizeof(guid)); //((uint8*)&guid);
			sLog->outInfo(LOG_FILTER_NETWORKIO, "SetGuid guid : %u _guidBytes %u", guid, _guidBytes);
		}

		void SetGuildGuid(uint32 guildGuid)
		{
            memcpy(_guildGuidBytes, &guildGuid, sizeof(guildGuid)); //_guildGuidBytes = ((uint8*)&guildGuid);
			sLog->outInfo(LOG_FILTER_NETWORKIO, "SetGuildGuid guildGuid : %u _guidBytes %u", guildGuid, _guildGuidBytes);
		}
