    ...
    */
    ...
    bool StaticMapTree::InitMap(const std::string &fname, VMapManager2* vm)
    {
        ...
        if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
        {
            ...
            {
                success = false;
                VMAP_ERROR_LOG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : could not acquire WorldModel pointer for '%s'", spawn.name.c_str());
            }
        }
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        if (!iTreeValues)
        {
            VMAP_ERROR_LOG(LOG_FILTER_MAPS, "StaticMapTree::LoadMapTile() : tree has not been initialized [%u, %u]", tileX, tileY);
            return false;
        }
        bool result = true;
        ...
        if (tf)
        {
            ...
            for (uint32 i=0; i<numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name);
                    if (!model)
                        VMAP_ERROR_LOG(LOG_FILTER_MAPS, "StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [%u, %u]", tileX, tileY);
    ...
    void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        {
            VMAP_ERROR_LOG(LOG_FILTER_MAPS, "StaticMapTree::UnloadMapTile() : trying to unload non-loaded tile - Map:%u X:%u Y:%u", iMapID, tileX, tileY);
            return;
        }
                ...
                for (uint32 i=0; i<numSpawns && result; ++i)
                {
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            ...
                            VMAP_ERROR_LOG(LOG_FILTER_MAPS, "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
