...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    stmt->setUInt64(++index, _experience);
    stmt->setUInt64(++index, _todayExperience);
    trans->Append(stmt);
...
void Guild::SaveXP()
{
    if (getMSTime() - m_lastXPSave >= 60000) // 1 minute. Hardcoded value for now
    {
        ...
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_EXPERIENCE);
        stmt->setUInt64(0, _experience);
        stmt->setUInt64(1, _todayExperience);
        stmt->setUInt32(2, uint32(_level));
...
void Guild::AddGuildNews(uint8 type, uint64 guid, uint32 flags, uint32 value)
{
    ...
    WorldPacket data(SMSG_GUILD_NEWS_UPDATE, 40);
