...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    bool instantLogout = GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) && !GetPlayer()->isInCombat() ||
                         GetPlayer()->isInFlight() || GetSecurity() >= AccountTypes(sWorld->getIntConfig(CONFIG_INSTANT_LOGOUT)) || GetPlayer()->GetMapId() == 974 || GetPlayer()->GetZoneId() == 5861;
    ...
    if (GetPlayer()->isInCombat() && !canLogoutInCombat)
        reason = 1;
    else if (GetPlayer()->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING | MOVEMENTFLAG_JUMPING))
