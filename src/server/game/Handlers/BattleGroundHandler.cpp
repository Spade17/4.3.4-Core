    ...
    else // leave queue
    {
        ...
        if (ginfo.IsRated && ginfo.IsInvitedToBGInstanceGUID)
        {
            ...
            if (at)
            {
                ...
                at->MemberLost(_player, ginfo.ArenaMatchmakerRating, ginfo.OpponentsMatchmakerRating);
                at->SaveToDB();
            }
        }
    ...
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        ...
        if (bgTypeId == _player->GetBattlegroundTypeId())
        {
            ...
            if (bg && bg->GetArenaType() == arenaType)
            {
                ...
                sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, GetPlayer(), i, STATUS_IN_PROGRESS, bg->GetElapsedTime(), arenaType, _player->GetTeam());
                SendPacket(&data);
                continue;
            }
        }
