    ...
    if (cFamily && petType == HUNTER_PET)
    {
        ...
        if (getLevel() > 70)
        {
            if (cinfo->type_flags & CREATURE_TYPEFLAGS_EXOTIC)
                if (getLevel() > 85)
                    maxlevel = 85; //for gms and fun servers
                else
                    maxlevel = getLevel();
            else
                maxlevel = 70;
        ...
        scale = minscale + (maxlevel * ((maxscale - minscale) / 85));
    ...
    switch (petType)
    {
        ...
        default:
        {
            ...
            switch (GetEntry())
            {
                ...
                case 36070: //force of nature
                {
                    if (!pInfo)
                        SetCreateHealth(30 + 30*petlevel);
                    float bonusDmg = GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE) * 0.077f;
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 5)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 5)));
                    break;
                }
