    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            else
            {
                ...
                if(aura->GetId() == 33763 || aura->GetId() == 94447)
                {
...
{
    SpellInfo const* soulSwapDots = sSpellMgr->GetSpellInfo(92794);
    ...
    if (target && m_sourceSoulSwapDots != target->GetGUID())
    {
        ...
        for (uint8 i = 0; i < 7; ++i)
        {
            ...
            {
                if(SpellInfo const* soulSwapAuraProto = sSpellMgr->GetSpellInfo(m_soulSwapAuras[i]))
                {
                    if(soulSwapAuraProto->SpellFamilyName == SPELLFAMILY_WARLOCK && soulSwapDots)
                    {
                        flag96 affDots = soulSwapDots->Effects[EFFECT_0].SpellClassMask;

                        if (soulSwapAuraProto->SpellFamilyFlags & affDots)
                        {
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 99262:
                {
                    if(victim->HasAura(99252))
                    {
                        if(Player* player = ToPlayer())
                        { 
                            if (AuraEffect* aurEff = player->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_GENERIC, 2302, EFFECT_0))
                            {
                                uint32 stack = aurEff->GetBase()->GetStackAmount();
                                int bp0 = 5*stack;
                                player->CastCustomSpell(player, 99263, &bp0, NULL, NULL, false);
                            }
                        }
                    }
                    break;
                ...
                case 100232:
                {
                    uint32 stack = triggeredByAura->GetBase()->GetStackAmount();
                    
                    if(Player* player = ToPlayer())
                    {
                        // In 25 player Heroic Difficulty the healer gains an application of Vital Spark for every five stacks of Torment.
                        if(player->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
                            stack = uint32(stack/5);
                        else
                            stack = uint32(stack/3);
                        
                        if(AuraEffect* aurEff = player->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_GENERIC, 2302, EFFECT_0))
                            stack += aurEff->GetBase()->GetStackAmount();

                        if(stack > 0)
                            victim->SetAuraStack(99262, victim, stack);
                    }
                    break;
                }
                // Weight of a Feather
                case 96879:
                {
                    if (GetHealthPct() < 100.0)
                        return false;

                    int32 amount = damage;
                    triggered_spell_id = 96881;

                    if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_GENERIC, 2247, 0))
                    {
                        if (Aura* weight = GetAura(96881))
                        {
                            if (weight->GetEffect(0))
                                amount += weight->GetEffect(0)->GetAmount();
                        }
                    ...
                    CastCustomSpell(this, triggered_spell_id, &amount, NULL, NULL, true);
                    return true;
                    break;
                }
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (dummySpell->SpellIconID)
            {
                ...
                case 4625:
                    ...
                    break;
                // Magic Absorption
                case 459: // only this spell has SpellIconID == 459 and dummy aura
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                case 12725:
                    ...
                    break;
				case 107987: //Soulshifter Vortex
					if(GetOwner()->getClass() == CLASS_MAGE)
					{
						triggered_spell_id = triggeredByAura->GetAmount()/2;
						basepoints0 = triggeredByAura->GetMiscValue();
					}
					else if(GetOwner()->getClass() == !CLASS_MAGE)
					{
                        triggered_spell_id = triggeredByAura->GetAmount();
                        basepoints0 = triggeredByAura->GetMiscValue();
                    }
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 91986:
                {
                    ...
                    {
                        float thresholdMod = dummySpell->Id == 85112 ? 1.4f : 0.7f;
                        int32 dmgPastSecs = GetDamageDoneInPastSecs(7);
                        if(this->ToPlayer() && this->ToPlayer()->GetPet())
                        {
                            dmgPastSecs += this->ToPlayer()->GetPet()->GetDamageDoneInPastSecs(7);
                        }

                        int32 bpDamage = int32(ApplyPct(dmgPastSecs, triggerAmount) / 7);
                        int32 dmgThreshold = int32(SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * thresholdMod / 7);
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 33882:
                {
					if (Player* caster = ToPlayer())
                    {
						if (HealthAbovePct(50) || !caster->HasSpellCooldown(45281))
							return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch (dummySpell->Id)
            {
                ...
                // Retribution t12 2p bonus
                case 99093:
                {
                    triggered_spell_id = 99092;
                    target= victim;
                    basepoints0 = damage *15 /100 /2;
                    break;
                }
                // Selfless Healer
                case 85803:
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 16235:
                {
                    ...
                    int32 threshold = int32(victim->GetMaxHealth() * 10 / 100);

                    if (Aura* aura = victim->GetAura(105284, GetGUID()))
                    {
                        ...
                        if(AuraEffect* aurEff = aura->GetEffect(EFFECT_0))
                        {
                            ...
                            aurEff->ChangeAmount(bp0 > threshold ? threshold : bp0);
                        }
                    }
                    else
                    {
                        bp0 = bp0 > threshold ? threshold : bp0;
    ...
    switch (dummySpell->Id)
    {
        ...
        case 93099:
        {
            ...
            if(Player* player = triggeredByAura->GetCaster()->ToPlayer())
            {
                // For druid don't proc if we aren't in bear form
                if (dummySpell->Id == 84840 && GetShapeshiftForm() != FORM_BEAR)
                    return false;

                // Disale vengeance in BG/Arena/PvP
                if (player->InArena() || player->InBattleground() || player->IsPvP())
                    return false;
                ...
                if (player->duel)
                    return false;
                int32 bp;
                if(dummySpell->Id == 50720)
                    bp = int32(damage * 0.2f);
                else
                    bp = int32(damage * 0.05f);

                if (AuraApplication* aurApp = player->GetAuraApplication(76691, player->GetGUID(), 0, 0, 0))
                {
                    bp += aurApp->GetBase()->GetEffect(0)->GetAmount();
                    if (bp <= (player->GetMaxHealth() / 10.0f))
                    {
                ...
                if (bp <= (player->GetMaxHealth() / 10.0f) )
                    player->CastCustomSpell(player, 76691, &bp, &bp, 0, true);
                return true;
            }
            break;
        }
        ...
        case SPELLFAMILY_ROGUE:
            switch(dummySpell->Id)
            {
                ...
                case 1776:
                    *handled = true;
				    AttackStop();
					InterruptSpell(CURRENT_AUTOREPEAT_SPELL); // break Auto Shot and autohit
					InterruptSpell(CURRENT_CHANNELED_SPELL);  // break channeled spells
                    if(procSpell)
                    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 99204:
                    if(Player* player = this->ToPlayer())
                    {
                        ...
                        {
                            player->ReduceSpellCooldown(2894, 4000);
                        }
                    }
                    break;
                default:
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    if (sSpellMgr->GetSpellInfo(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
            {
                switch (auraSpellInfo->Id)
                {
                    // Druid t12 2p bonus resto
                    case 99013:
                    {
                        trigger_spell_id = 99007;
                        break;
                    }
                    // Efflorescence
                    case 34151:
            ...
            case SPELLFAMILY_SHAMAN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    {
						if (Player* shaman = ToPlayer())
						{
							if (!shaman->HealthBelowPctDamaged(30, damage) || shaman->HasSpellCooldown(31616))
								return false;

							basepoints0 = int32(CountPctFromMaxHealth(triggerAmount));
							target = this;
							shaman->CastCustomSpell(shaman, 31616, &basepoints0, NULL, NULL, true, 0, 0);                
							shaman->AddSpellCooldown(31616, 0, time(NULL) + 30);
                            if (victim && victim->isAlive())
                ...
                if (auraSpellInfo->Id == 70656)
                {
                    ...
                    for (uint8 i = 0; i < MAX_RUNES; ++i)
                        if (ToPlayer()->GetRuneCooldown(i) == 0)
                            return false;
                }//Item - Death Knight T12 Melee 2P Bonus
                else if (auraSpellInfo->Id == 98970)
                {
                    trigger_spell_id = 98971;
                }//Item - Death Knight T12 Tank 2P Bonus
                else if (auraSpellInfo->Id == 98956)
                {
                    trigger_spell_id = 98957;
                }
                break;
            }
            case SPELLFAMILY_ROGUE:
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 91702:
            ...
            {
                if(AuraEffect* aurEff = owner->GetDummyAuraEffect(SPELLFAMILY_WARLOCK, 1982, EFFECT_2))
                {
                    int32 bp0 = aurEff->GetAmount();
                            
                    // path 4.1.0: Now restores more mana (four times as much) when the warlock is using a Felguard or Felhunter.
                    if(this->GetEntry() == 17252 || this->GetEntry() == 417)
                    {
                        bp0 *= 4;
                    }
		...
		case 105552: // Kiss of death
        {
            if (Player* dk = ToPlayer())
            {
				if (!dk->HealthBelowPctDamaged(35, damage) || dk->HasSpellCooldown(trigger_spell_id))
                    return false;

				dk->CastCustomSpell(dk, trigger_spell_id, &basepoints0, NULL, NULL, true, 0, 0);
                dk->AddSpellCooldown(trigger_spell_id, 0, time(NULL) + 45);
                return true;
            }
            break;
        }
		...
		case 125047: // Glyph of the treant
		{
			if (Player* druid = ToPlayer())
			{
				if (druid->HasSpellCooldown(59428))
                    return false;
			
				druid->CastCustomSpell(druid, 59428, &basepoints0, NULL, NULL, true, 0, 0);
				druid->AddSpellCooldown(59428, 0, time(NULL) + 1);
				return true;
			}
			break;
		}	
		case 61329:
		{
			if (Player* player = ToPlayer())
			{
				player->CastCustomSpell(player, trigger_spell_id, &basepoints0, NULL, NULL, true, 0, 0);
                return true;
            }
            break;
        }
		case 109832: //Souldrinker
		case 107895: //Souldrinker
		{
			if (Player* player = ToPlayer())
			{
				...
				if(roll_chance_i(15))
					player->CastCustomSpell(player, trigger_spell_id, &basepoints0, NULL, NULL, true, 0, 0);
				player->AddSpellCooldown(trigger_spell_id, 0, time(NULL) + 15);
                return true;
            }
            break;
        }
    ...
    switch (trigger_spell_id)
    {
        ...
        case 44544:
            if (procSpell)
                if (procSpell->Id == 82739 || procSpell->Id == 95969) 
                    return false;
            break;
        // Focused Will 
        case 45242:
        ...
        case 51945: // Earthliving Weapon
        {
            ...
            if (victim)
            {
                ...
                {
                    this->CastSpell(victim, trigger_spell_id);
                    return true;
                }
            }
            else
            ...
            if(this->isPet())
            {
                if(Unit* owner = this->GetCharmerOrOwner())
                {
                    switch(spellProto->Id)
                    {
                        ...
                        // Whiplash
                        case 6360:
                            DoneTotal += (float)owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.425f;
                            break;
                        // Firebolt
                        case 3110:
                            DoneTotal += (float)owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.309f;
                            break;
                        // Doom Bolt
                        case 85692:
                            DoneTotal += (float)owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 1.36f;
                            break;
                        // Torment
                        case 3716:
                            DoneTotal += (float)owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.256f;
                            break;
                    }
                }
            }
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    case SPELLFAMILY_DRUID:
                        ...
                        {
                            // Predatory Strikes
                            if (AuraEffect const* aurEff = GetDummyAuraEffect(SPELLFAMILY_DRUID, 1563, 0))
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            switch(spellProto->Id)
            {
                // Healing Death Coil
                case 47633:
                    healamount += CalculatePct(int32(this->GetTotalAttackPowerValue(BASE_ATTACK)), 35);
                    AddPct(DoneTotalMod, 125);
                    break;
            }
            break;
        case SPELLFAMILY_PALADIN:
        ...
        switch (spellProto->SpellFamilyName)
        {
                ...
                switch(spellProto->Id)
                {
                    ...
                    case 8676:
                        ...
                        if(Player * player = this->ToPlayer())
                        {
                            ...
                            int32 weaponDmg = this->CalculateDamage(BASE_ATTACK, true, false) * (1.99f * mod); //default is 1.9 but increased it a little because of it's low damage & complain fix.
            ...
            case SPELLFAMILY_PALADIN:
                switch(spellProto->Id)
                {
                    ...
                    case 85256:
                    {
                        int32 damage = this->CalculateDamage(BASE_ATTACK, true, true);
                ...
                if(this->isPet())
                {
                    if(Unit* owner = this->GetCharmerOrOwner())
                    {
                        switch(spellProto->Id)
                        {
                            // Felstorm
                            case 89753:
                                pdamage += (float)owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.231f;
                                break;
                            // Legion Strike
                            case 30213:
...
void Unit::Dismount(bool from_aura)
{
    if (!IsMounted())
        return;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    // Firelands hack - Todo find a better way
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
		...
		// Stealth should not be removed on absorbs.
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
	...
	{
		// If both charmer and target are players charmer goes into pvp
...
void Unit::ApplyResilience(Unit const* victim, int32* damage) const
{
    ...
    const Unit *target = victim->ToPlayer();
    if (!target)
    {
        target = victim->ToCreature();
        if (target)
        {
...
void Unit::RewardRage(uint32 baseRage, bool attacker, bool isPhysical)
{
    ...
    else
    {
        ...
        if (isPhysical)
        {
            ...
            baseRage += GetPlayerDamageReduction(baseRage);
            baseRage *= mod;
