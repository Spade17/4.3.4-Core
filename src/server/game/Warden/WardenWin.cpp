...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "%s failed hash reply. Action: %s", _session->GetPlayerName().c_str(), Penalty().c_str());
        return;
    }
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    {
        buff.rpos(buff.wpos());
        sLog->outWarn(LOG_FILTER_WARDEN, "%s failed checksum. Action: %s", _session->GetPlayerName().c_str(), Penalty().c_str());
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            sLog->outWarn(LOG_FILTER_WARDEN, "%s failed timing check. Action: %s", _session->GetPlayerName().c_str(), Penalty().c_str());
            return;
        }
    ...
    if (checkFailed > 0)
    {
        ...
        sLog->outWarn(LOG_FILTER_WARDEN, "%s failed Warden check %u. Action: %s", _session->GetPlayerName().c_str(), checkFailed, Penalty(check).c_str());
    }
