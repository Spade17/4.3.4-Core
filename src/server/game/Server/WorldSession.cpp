...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket& p) const
{
    ...
    sLog->outError(LOG_FILTER_NETWORKIO, "AntiDOS: Account %u, IP: %s, sent unacceptable packet (opc: %u, size: %u)",
    ...
    switch (_policy)
    {
        case POLICY_LOG:
            return true;
        case POLICY_KICK:
            sLog->outError(LOG_FILTER_NETWORKIO, "AntiDOS: Player kicked!");
            return false;
        case POLICY_BAN:
        {
            ...
            sLog->outError(LOG_FILTER_NETWORKIO, "AntiDOS: Player automatically banned for %u seconds.", duration);
