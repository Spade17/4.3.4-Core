...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
	...
	if (!target)
    {
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set NULL target, effect %u", m_spellInfo->Id, effIndex);
		return;
	}
    ...
    switch (targetType.GetObjectType())
    {
        case TARGET_OBJECT_TYPE_UNIT:
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected unit, got %s, effect %u", m_spellInfo->Id, GetLogNameForGuid(target->GetGUID()), effMask);
                return;
            }
            break;
        }
        case TARGET_OBJECT_TYPE_GOBJ:
            ...
            {
                sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected gameobject, got %s, effect %u", m_spellInfo->Id, GetLogNameForGuid(target->GetGUID()), effMask);
                return;
            }
            break;
        case TARGET_OBJECT_TYPE_DEST:
    ...
    if (!unitTargets.empty())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
                ...
                else if (m_spellInfo->Id == 740) // Tranquility
                {
                    maxSize = 5;
                    power = POWER_HEALTH;
                }
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    {
        // No spells with SPELL_ATTR3_NO_INITIAL_AGGRO trigger combat.
        // No spells with SPELL_ATTR1_NOT_BREAK_STEALTH trigger combat.
        // No spells with SPELL_ATTR1_NO_THREAT trigger combat.
        bool spellTriggersCombat = !(m_spellInfo->AttributesEx3 & SPELL_ATTR3_NO_INITIAL_AGGRO  ||
                                     m_spellInfo->AttributesEx  & (SPELL_ATTR1_NOT_BREAK_STEALTH | SPELL_ATTR1_NO_THREAT));

		// Only for players.
		// No spells with SPELL_ATTR4_FIXED_DAMAGE trigger combat.
		...
		if (spellTriggersCombat && m_caster->GetTypeId() == TYPEID_PLAYER)
			spellTriggersCombat = !(m_spellInfo->AttributesEx4 & SPELL_ATTR4_FIXED_DAMAGE                 ||                                   (m_spellInfo->AttributesEx4 & SPELL_ATTR4_TRIGGERED                    &&
			!(m_spellInfo->AttributesEx3 & SPELL_ATTR3_TRIGGERED_CAN_TRIGGER_PROC_2 ||
			m_spellInfo->AttributesEx2 & SPELL_ATTR2_TRIGGERED_CAN_TRIGGER_PROC)) ||
    ...
    if (spellHitTarget)
    {
        ...
        if (unit->IsPvP() && m_caster->GetTypeId() == TYPEID_PLAYER && !m_caster->HasAura(58984)) //shadowmeld should never trigger pvp
    ...
    if (std::vector<int32> const* spellTriggered = sSpellMgr->GetSpellLinked(m_spellInfo->Id + SPELL_LINK_HIT))
    {
        for (std::vector<int32>::const_iterator i = spellTriggered->begin(); i != spellTriggered->end(); ++i)
        {
            if (*i < 0)
                ...
                m_caster->CastSpell(unit, *i, true, 0, 0, m_caster->GetGUID());
        }
    }
}
...
void Spell::cancel()
{
	if (m_spellInfo->AttributesEx3 & SPELL_ATTR3_IGNORE_OTHER_CASTS)
		return;
...
void Spell::SendSpellGo()
{
    ...
    {
        // spells
        if (m_spellInfo->Id == 35395 || m_spellInfo->Id == 53385)
        {
            // Crusader Strike & Divine Storm benefit from haste Sanctity of Battle
            float haste = (2 - m_caster->ToPlayer()->GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_HASTE));
            int32 cooldown = 4500;
            int32 diffCool = 0;
            if (haste > 0)
            {
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    if (didHit)
	{
        int32 rp = int32(runeCostData->runePowerGain);
		if (rp)
		{
			if (Player* modOwner = m_caster->GetSpellModOwner())
				modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_COST, rp, this);

			rp = int32(sWorld->getRate(RATE_POWER_RUNICPOWER_INCOME) * rp);
			rp += m_caster->GetTotalAuraModifier(SPELL_AURA_MOD_RUNE_REGEN_SPEED) * rp / 100;
			if (rp > 0)
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly casted by a trigger)
            {
				...
				// Improve pathfinidng for charge"
...
SpellCastResult Spell::CheckPower()
{
	...
	//The art of war does not use mana
	if(m_spellInfo && m_spellInfo->PowerType >= POWER_MANA && (m_spellInfo->Id == 87138 || m_spellInfo->Id == 59578))
		return SPELL_CAST_OK;

	// Zealotry does not use holy power
	if(m_spellInfo->PowerType >= POWER_HOLY_POWER && m_spellInfo->Id == 85696)
		return SPELL_CAST_OK;
