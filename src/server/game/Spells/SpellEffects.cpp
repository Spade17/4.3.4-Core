...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS]=
{
    ...
    &Spell::EffectAddGuildBankTab,                          //173 SPELL_EFFECT_UNLOCK_GUILD_VAULT_TAB
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch (m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 51963:
                        if (Unit* caster = GetCaster())
                        {
                            if(caster->isGuardian())
                            {
                                ...
                                {
                                    damage += owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.33f;
                                }
                            }
                        }
                        break;
                    ...
                    case 69041:
                    {
                        damage =uint32(1+ (0.25f* m_caster->GetTotalAttackPowerValue(BASE_ATTACK))+ (0.429f* m_caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE))+ (m_caster->getLevel() * 2)+ (m_caster->GetStat(STAT_INTELLECT)* 0.50193f));
                    }
                }
                break;
            }
            case SPELLFAMILY_WARLOCK:
                switch (m_spellInfo->Id)
                {
                    ...
                    case 3110:
                        if(m_caster && m_caster->isPet() && m_caster->GetOwner())
                        {
                            Unit* creator = m_caster->GetOwner();

                            if(AuraEffect* aurEff = creator->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 5116, EFFECT_0))
                            {
                                if(effIndex == EFFECT_0)
                                {
                                    float thresholdMod = aurEff->GetId() == 85112 ? 1.4f : 0.7f;
                                    int32 dmgDonePast = creator->GetDamageDoneInPastSecs(7);
                                    dmgDonePast += m_caster->GetDamageDoneInPastSecs(7);
                                    int32 bpDamage = int32(ApplyPct(dmgDonePast, aurEff->GetAmount()) / 7);
                                    int32 dmgThreshold = int32(creator->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * thresholdMod / 7);
                ...
                switch(m_spellInfo->Id)
                {
                    case 6343: // Thunder Clap
                    {
                        if(m_caster->GetTypeId() == TYPEID_PLAYER)
                            if (Unit* trg2 = m_caster->ToPlayer()->GetSelectedUnit())
                                if(trg2->HasAura(94009))
                                    if ((m_caster->HasAura(84614) && roll_chance_i(50)) || m_caster->HasAura(84615)) // Blood and Thunder (Rank 1 & 2)
                    ...
                    case 46968: 
                    {
                        int32 pct = m_caster->CalculateSpellDamage(unitTarget, m_spellInfo, EFFECT_2);
                        if (pct > 0)
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            switch (m_spellInfo->Id)
            {
                case 46584: // Raise Dead
                    ...
                    m_caster->ToPlayer()->RemoveSpellCooldown(bp, true);
                    m_caster->CastSpell(unitTarget, bp, true);
                    break;
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
	...
	{
		sLog->outError(LOG_FILTER_SPELLS_AURAS, "Spell::EffectTeleportUnits - spellId %u attempted to teleport creature to a different map.", m_spellInfo->Id);
		return;
	}
        ...
        case SPELLFAMILY_WARLOCK:
            if (m_spellInfo->Category == SPELLCATEGORY_DEVOUR_MAGIC)
            {
                if (Unit* owner = m_caster->GetOwner())
                {
                    ...
                    heal_amount += owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.15f;
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            if (m_spellInfo->Id == 53)
            {
				...
				totalDamagePercentMod *= 2.0f;         // 200% damage
            }
            break;
        }
        case SPELLFAMILY_SHAMAN:
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            break;
			if (m_spellInfo->Id == 3044)             // Arcane Shot
			{
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
                ...
                case 45902:
                    {
                        ...
                        // Death Knight T12 Tank 2P Bonus
                        if(unitTarget->HasAura(98957) && GetCaster() && unitTarget->GetAura(98957,GetCaster()->GetGUID()))
                            bonusPct = 25;
            ...
            if (m_spellInfo->SpellIconID == 1736)
            {
                // Death Knight T12 Tank 2P Bonus
                if(unitTarget->HasAura(98957) && GetCaster() && unitTarget->GetAura(98957,GetCaster()->GetGUID()))
                    AddPct(totalDamagePercentMod, 100);
                else
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 99489:
                {
                    if(unitTarget && unitTarget->GetMap()->IsHeroic())
                    {
                        if(AuraEffect* aurEff = m_caster->GetAuraEffect(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELLFAMILY_GENERIC, 173, EFFECT_0))
                        {
                            uint32 stack = aurEff->GetBase()->GetStackAmount();
                            m_caster->SetAuraStack(aurEff->GetId(), unitTarget, stack);
                        }
                    }
                    break;  
                }
                // Death Grip (Echo of Sylvanas)
                case 101397:
                    unitTarget->CastSpell(m_caster, m_spellInfo->Effects[effIndex].CalcValue(), false);
                    break;
                // Energy Storm
                case 42577:
                    if (!unitTarget)
...
void Spell::EffectAddGuildBankTab(SpellEffIndex effIndex)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_LAUNCH)
        return;
    ...
    if (!m_caster->ToPlayer()->GetGuildId())
        return;

    uint32 tabId = GetSpellInfo()->Effects[effIndex].BasePoints;
