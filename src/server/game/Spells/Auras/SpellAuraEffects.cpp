...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleAuraModIncreaseSpeed,                      //335 SPELL_AURA_335
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            // Leader of the Pack
            if (plrTarget->GetDummyAuraEffect(SPELLFAMILY_DRUID, 312, EFFECT_0))
            {
    ...
    else
    {
        ...
        {
            // Remove Jinx power cost debuff
           if (AuraEffect* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT, SPELLFAMILY_GENERIC, 5002, EFFECT_0))
           {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    else
    {
        ...
        switch (form)
        {
            ...
            case FORM_BERSERKERSTANCE:
            {
                // Moved to spellscript "spell_warr_stance_handler"
                /*int32 Rage_val = 0;
                // Defensive Tactics
                if (GetMiscValue() == FORM_DEFENSIVESTANCE)
                {
                ...
                {
                    target->SetPower(POWER_RAGE, Rage_val / 10);
                }
                break;*/
            }
            default:
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
		...
		if(target->HasAura(51722) || target->HasAura(676) || target->HasAura(118093) || target->HasAura(91644) || target->HasAura(64044))
		{
			target->RemoveAura(19263);
		}
    }
...
void AuraEffect::HandleModFear(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 max_damage = int32(target->GetCreateHealth()*0.10f);
    int32 breakChance=irand(20,100);
    int32 m_amount = max_damage*((float)breakChance)/100.0f;
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        ...
        case 54508: // Demonic Empowerment
            mechanic =  (1 << MECHANIC_STUN) | (1 << MECHANIC_SNARE) | (1 << MECHANIC_FEAR) | (1 << MECHANIC_BANISH) | (1 << MECHANIC_HORROR) | (1 << MECHANIC_ROOT);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_STUN, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SNARE, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_FEAR, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_BANISH, apply);
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	if (m_spellInfo->Id == 31884) // Avenging Wrath
		if(target->HasAura(53375) || target->HasAura(90286) || target->HasAura(53376)) // Santified Wrath rank 1, 2, 3
			target->CastSpell(target,57318,true); // Allow use Hammer of Wrath
}
    ...
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (GetSpellInfo()->Id)
            {
				...
				// Smoke Bomb
                case 88611:
					if (target->IsHostileTo(caster) && target->HasAuraType(SPELL_AURA_MOD_STEALTH))
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    // Dark Intent hot hack
    if(caster->HasAura(85767) && crit)
    {
        caster->DarkIntentHandler();
    }
    ...
    if (target != caster && GetSpellInfo()->AttributesEx2 & SPELL_ATTR2_HEALTH_FUNNEL)
    {
        ...
        CleanDamage cleanDamage = CleanDamage(0, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
        caster->DealDamage(caster, funnelDamage, &cleanDamage, DOT, GetSpellInfo()->GetSchoolMask(), GetSpellInfo(), true);
    }
