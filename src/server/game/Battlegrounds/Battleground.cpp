...
inline void Battleground::_ProcessJoin(uint32 diff)
{
	...
	// 1v1 Arena - Start arena after 1 minute or 30 seconds, when all players are in arena
	if(GetArenaType() == ARENA_TYPE_5v5 && GetStartDelayTime() > StartDelayTimes[BG_STARTING_EVENT_SECOND] && (m_PlayersCount[0] + m_PlayersCount[1]) == 2)
		SetStartDelayTime(StartDelayTimes[BG_STARTING_EVENT_SECOND]);
        ...
        if (mapid != 607)
        {
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            {
                if (Player *player = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    if ((player->GetDistance(x,y,z) >= 75.0f && mapid != 628 && mapid != 30) || (player->GetDistance(x,y,z) >= 200.0f && mapid == 628) || (player->GetDistance(x,y,z) >= 100.0f && mapid == 30))
                    {
                        sLog->outError(LOG_FILTER_BATTLEGROUND, "BATTLEGROUND: Sending %s back to start location (possible exploit) map %u, X %f, Y %f, Z %f, O %f", player->GetName(), mapid, x, y, z, O);
        ...
        if (isArena())
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    sBattlegroundMgr->BuildBattlegroundStatusPacket(&status, this, player, queueSlot, STATUS_IN_PROGRESS, player->GetBattlegroundQueueJoinTime(m_TypeID), GetArenaType(), player->GetTeam());
        ...
        if (team == winner)
        {
            ...
            if (!guildAwarded)
            {
                ...
                if (uint32 guildId = GetBgMap()->GetOwnerGuildId(player->GetTeam()))
                    ...
                    {
						uint32 guildXP = uint32(27900 * 0.25);
						uint32 guildRep = uint32(27900 * guildXP / 450);

						if (Guild* guild = sGuildMgr->GetGuildByGuid(player->GetGuildId()))
							guild->GiveXP(guildXP, player);
						if (FactionEntry const* guildEntry = sFactionStore.LookupEntry(1168))
							player->GetReputationMgr().ModifyReputation(guildEntry, guildRep);
...
void Battleground::PlayerAddedToBGCheckIfBGIsRunning(Player* player)
{
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, player, player->GetBattlegroundQueueIndex(bgQueueTypeId), STATUS_IN_PROGRESS, GetElapsedTime(), GetArenaType(), player->GetTeam());
    player->SendDirectMessage(&data);
}
