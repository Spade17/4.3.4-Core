/*Copyright (C) 2012 SkyMist Project (starting framework wrote by Naios).
*
* Script done. TODO:
*
* - Live Testing needed.
...
class boss_majordomus : public CreatureScript
{
	...
	{
		boss_majordomusAI(Creature* c) : BossAI(c, DATA_MAJORDOMO_STAGHELM)
		{
		...
		void Reset()
		{
			if(instance)
            {
				instance->SetBossState(DATA_MAJORDOMO_STAGHELM_EVENT, NOT_STARTED);
        ...
        void EnterEvadeMode()
        {
            ...
            if (instance)
                instance->SetBossState(DATA_MAJORDOMO_STAGHELM_EVENT, FAIL);
		...
		void JustDied(Unit * /*victim*/)
		{
			if (instance)
			{
			    instance->SetBossState(DATA_MAJORDOMO_STAGHELM_EVENT, DONE);
                instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me); // Remove
            }
		...
		void EnterCombat(Unit* who)
		{
			if(instance)
            {
				instance->SetBossState(DATA_MAJORDOMO_STAGHELM_EVENT, IN_PROGRESS);
