...
public:
    ...
    struct boss_warlordAI: public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            events.ScheduleEvent(EVENT_FOCUSEF_ANGER, 6000);
		...
		void EnterEvadeMode()
		{
			...
			instance->SetData(DATA_PORTALS_ON_OFF, DONE);
			DespawnCreatures(NPC_EYE_1);
			DespawnCreatures(NPC_SCOURGE_1);
			DespawnCreatures(NPC_CLAW_1);
			DespawnCreatures(NPC_SPHERE);
			events.Reset();
        ...
        void UpdateAI(uint32 diff)
        {
			...
			if (sprey && me->HasAura(AURA_DIFFUSION))
			{
				...
				// Summon Eye
                for (int8 i = 0; i < 7; i++)
				    me->SummonCreature(NPC_EYE_1, eye[i], TEMPSUMMON_MANUAL_DESPAWN);

				if (IsHeroic()) // Heroic 10 and 25
				{
					// Summon Scourge
					for (int8 i = 0; i < 8; i++)
					    me->SummonCreature(NPC_SCOURGE_1, scourge[i], TEMPSUMMON_MANUAL_DESPAWN);

					// Summon Claw
					for (int8 i = 0; i < 3; i++)
					    me->SummonCreature(NPC_CLAW_1, claw[i], TEMPSUMMON_MANUAL_DESPAWN);
				}
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FOCUSEF_ANGER:
                    ...
                    events.ScheduleEvent(EVENT_FOCUSEF_ANGER, 10500);
                    break;

                case EVENT_PSYCHIC_DRAIN:
		            DoCast(SPELL_PSYCHIC_DRAIN);

                    events.ScheduleEvent(EVENT_PSYCHIC_DRAIN, 16500);
                    break;
				...
				case EVENT_SPHERE:
					...
					{
						DoSummon(NPC_SPHERE, maw(), 40.0f, 0, TEMPSUMMON_MANUAL_DESPAWN);
						sprey = true;
					}
		...
		Creature* maw()
		{
			return me->FindNearestCreature(55544, 100.0f, true);
		}
    };
};
...
public:
	...
	struct npc_sphereAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			if (Player* target = me->FindNearestPlayer(5.0f, true))
			{
				if (target->FindNearestCreature(me->GetEntry(), 4.0f, true) && !player)
				{
					player = true;
					boss = true;
					maw = true;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
				...
				case SPHERE_START:
					...
					events.ScheduleEvent(SPHERE_END, urand(30000, 60000));
					break;

				case SPHERE_END:
					events.CancelEvent(SPHERE_TARGET);
					me->SetSpeed(MOVE_RUN, 5.0f);
					if (roll_chance_i(50)) // player
					{
						player = false;
						boss = true;
						maw = true;
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
							me->GetMotionMaster()->MoveFollow(target, 0.1f, target->GetFollowAngle());
					}
					else if (roll_chance_i(50)) // boss
					{
						player = true;
						boss = false;
						maw = true;
						if (Creature* warlord = me->FindNearestCreature(NPC_WARLORD, 1000.0f, true))
							me->GetMotionMaster()->MoveFollow(warlord, 0.1f, warlord->GetFollowAngle());
					}
					else
					{
						player = true;
						boss = true;
						maw = false;
						if (Creature* tentacle = me->FindNearestCreature(57836, 1000.0f, true))
							me->GetMotionMaster()->MovePoint(0, tentacle->GetPositionX(), tentacle->GetPositionY(), tentacle->GetPositionZ());
					}
					break;
		...
		void UpdateAI(uint32 diff)
		{
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
				...
				case EVENT_SHADOW_GAZE:
					...
					events.ScheduleEvent(EVENT_SHADOW_GAZE, urand(1000, 3100));
					break;
				}
			}
		}
