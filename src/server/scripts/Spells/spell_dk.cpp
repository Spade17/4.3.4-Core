        ...
        class spell_dk_necrotic_strike_AuraScript : public AuraScript
        {
            ...
            void HandleEffectCalcAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
            {
                ...
                if (caster && target)
                {
                    ...
                    toAdd -= target->GetPlayerDamageReduction(toAdd);
    ...
    class spell_dk_death_strike_SpellScript : public SpellScript
    {
        ...
        void HandleOnCast()
        {
            if(Unit * caster = GetCaster())
            {
                ...
                uint32 damageTaken = caster->GetDamageTakenInPastSecs(5) * 0.2f;
        ...
        void HandleHit(SpellEffIndex /*effIndex*/)
        {
            if(Unit * caster = GetCaster())
            {
                if (caster->ToPlayer()->HasAuraType(SPELL_AURA_MASTERY) && caster->HasAura(48263))
                {
                    ...
                    {
						int32 shield = int32(bp * (50.0f + (6.25f * caster->ToPlayer()->GetMasteryPoints())) / 100.0f);
