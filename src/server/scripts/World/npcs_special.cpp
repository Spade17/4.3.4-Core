...
enum Infernal
{
    INFERNAL_DEMONIC_IMMOLATION_PASSIVE = 4525,
};

class npc_infernal : public CreatureScript
{
public:
    npc_infernal() : CreatureScript("npc_infernal") { }

    struct npc_infernalAI : PetAI
    {
        npc_infernalAI(Creature* creature) : PetAI(creature) {}
        ...
        void InitializeAI()
        {
            ...
            me->CastSpell(me, INFERNAL_DEMONIC_IMMOLATION_PASSIVE, true);
        ...
        {   
            if(checkTimer <= diff)
            {
                if(Unit* owner = me->GetOwner())
                {
                    ...
                    Unit* ownerVictim = isTargetable(player->GetSelectedUnit()) ? player->GetSelectedUnit() : NULL;
                    ...
                    // Infernal switches only when warlock cast bane of agony/doom on him. The Warlock baned target has an higher priority than the current Infernal target
                    if(infernalTarget)
                    {
        ...
        bool isTargetable(Unit* victim)
        {
            ...
            {
                return victim && 
                    (victim->HasAura(WARLOCK_BANE_OF_DOOM, owner->GetGUID()) 
                    || victim->HasAura(WARLOCK_BANE_OF_AGONY, owner->GetGUID()));
            }

            return false;
        }

        // Puts the Infernal in "dummy mode"
        void dummyGuard()
        {
            if(Unit* owner = me->GetOwner())
            {
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_infernalAI(creature);
    }
};
        ...
        {   
            if(checkTimer <= diff)
            {
                if(Unit* owner = me->GetOwner())
                {
                    ...
                    Unit* ownerVictim = isTargetable(player->GetSelectedUnit()) ? player->GetSelectedUnit() : NULL;
                    Unit* meVictim = isTargetable(me->getVictim()) ? me->getVictim() : NULL;
                    Unit* doomTarget = ownerVictim ? ownerVictim : meVictim ? meVictim : NULL;
                
                    // Doomguard switches only when warlock cast bane of Agony/Doom/Havoc on him. The Warlock baned target has an higher priority than the current Doomguard target
                    if(doomTarget)
                    {
                        me->Attack(doomTarget, true);
                        me->GetMotionMaster()->MoveChase(doomTarget);
                    }
                    else
                    {
                        // Reset victim so it wont't cast spell to incorrect targets
        ...
        bool isTargetable(Unit* victim)
        {
            if(Unit* owner = me->GetOwner())
            {
                return victim && 
                    (victim->HasAura(WARLOCK_BANE_OF_DOOM, owner->GetGUID()) 
                    || victim->HasAura(WARLOCK_BANE_OF_AGONY, owner->GetGUID())
                    || victim->HasAura(WARLOCK_BANE_OF_HAVOC, owner->GetGUID()));
            }
    ...
    struct npc_force_of_natureAI : PetAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if (AuraEffect* aurEff = owner->GetDummyAuraEffect(SPELLFAMILY_DRUID, 2681, EFFECT_0))
            {
                ...
                fungalGrowthSlowSpell = aurEff->GetMiscValue();
                           
                if (!fungalGrowthSlowSpell)
                    return;

                SpellInfo const* summonSpellInfo = sSpellMgr->GetSpellInfo(FORCE_OF_NATURE_FUNGAL_GROWTH_SUMMON);
                ...
                owner->CastSpell(
                    ...
                    pos.GetPositionY(), 
                    pos.GetPositionZ(), 
                    FORCE_OF_NATURE_FUNGAL_GROWTH_SUMMON, true);
                ...
                {     
                    ...
                    Trinity::AnyUnfriendlyCreatureInUnitRangeCheck check(owner, fungalGrowthCreID, 100.0f);
                    Trinity::CreatureListSearcher<Trinity::AnyUnfriendlyCreatureInUnitRangeCheck> searcher(owner, list, check);
                    owner->VisitNearbyGridObject(100.0f, searcher);
        ...
        struct npc_shadowfiendAI : public PetAI
        {
            ...
            void EnterCombat(Unit* victim)
            {
                if (me->isSummon())
                {
                    if (Unit* owner = me->ToTempSummon()->GetSummoner())
                    {
                        if (owner->HasAura(99154) && owner->HasAura(15473))
                            me->AddAura(99155, me);
                    }
                }
            }
        ...
        void BarrierChecker(Unit *who) 
        {
            ...
            if (who->isAlive() && who->HasAura(81782))
            {
                if (AuraEffect const* aur = who->GetAuraEffect(81782, 0))
                    aur->GetBase()->SetDuration(500, true);
            }
        }
